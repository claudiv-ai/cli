 ...
 */
export async function executeTransaction(ops, contextManifest, contextPath, newRefs, newFacts, scopePath) {
    const completed = [];
    try {
        ...
        const scope = contextManifest.scopes.find((s) => s.path === scopePath);
        if (scope) {
            for (const ref of newRefs) {
                const existing = scope.refs.findIndex((r) => r.file === ref.file);
                if (existing >= 0) {
        ...
        for (const op of completed.reverse()) {
            try {
                ...
                logger.error(`Rollback failed for ${op.file}: ${rollbackError.message}`);
            }
        }
        throw error;
    }
