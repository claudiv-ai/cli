...
import { diffCdml, getChangedElements, assembleContext, loadContextManifest, serializeContextManifest, executeClaudeHeadless, loadProject, parseSpecFile, detectPlanDirectives, parsePlanQuestions, allQuestionsAnswered, questionsToFacts, buildPlanPrompt, } from '@claudiv/core';
...
async function main() {
    ...
    if (isHeadless) {
        ...
        await processFileChange(config.specFile, content, '', // no cached version — treat everything as new
        projectRoot, registry, contextManifest, contextPath, config, isDryRun, scopeFilter);
    ...
    if (isWatch) {
        ...
        watcher.on('change', async (relativePath) => {
            try {
                ...
                await processFileChange(filePath, newContent, oldContent, projectRoot, registry, contextManifest, contextPath, config, isDryRun, scopeFilter);
    ...
    const content = await readFile(config.specFile, 'utf-8');
    await processFileChange(config.specFile, content, '', projectRoot, registry, contextManifest, contextPath, config, isDryRun, scopeFilter);
}
/**
 * Process a file change through the transactional pipeline.
 */
async function processFileChange(filePath, newContent, oldContent, projectRoot, registry, contextManifest, contextPath, config, isDryRun, scopeFilter) {
    // 1. Diff
    ...
    const filteredChanges = scopeFilter
        ? changes.filter((c) => c.path.includes(scopeFilter))
        : changes;
 ...
 */
async function processChange(change, projectRoot, registry, contextManifest, contextPath, config, isDryRun) {
 ...
 */
async function handlePlanDirectives($, filePath, projectRoot, config, contextManifest, contextPath) {
 ...
 */
async function handlePlanAnswers($, filePath, contextManifest, contextPath) {
 ...
 */
async function checkSystemComponents(content, projectRoot) {
    try {
